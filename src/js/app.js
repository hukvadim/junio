    ...
    return this.each(function() {
        ...
        if (body.length > options.length + options.minTrail) {
            ...
            if (splitLocation != -1) {
                ...
                obj.html(str1 + '<span class="js-read-more__ellipsis">' + options.ellipsisText +
                    '</span>' + '<span  class="js-read-more__trim">' + str2 + '</span>');
                obj.find('.js-read-more__trim').slideUp();

                // вставляем ссылку "читать полностью" в конец сцществующего содержимого
                obj.append(
                    '<a href="#" class="js-read-more-btn">' + options.moreText + '</a>'
...
(function(){
    ...
    const checkboxAll = $('.catalog__sorting .checkbox__input');
